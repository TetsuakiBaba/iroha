...
from iroha_dataset.download.http import DownloadError, RateLimiter, http_get
from iroha_dataset.jsonlio import dump_line, read_jsonl
    ...
    def documents(self) -> Iterator[Document]:
        ...
        if not self.jsonl_dir.exists() and not self.xml_dir.exists():
            raise FileNotFoundError(
                ...
                "先に `python -m iroha_dataset download --source kaken` を実行する")
        min_year = int(self.source_cfg.get("min_award_year", 2016))
