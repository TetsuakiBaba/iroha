...
"${PY}" -m iroha_dataset download --config "${CONFIG}" --set raw_dir="${WORK}/raw"
...
build() {   # $1 = balanced|full
  ...
  for s in ${SOURCES}; do
    ...
    "${PY}" -m iroha_dataset preprocess --config "${CONFIG}" --set raw_dir="${WORK}/raw" \
  ...
  echo "==> 読み一覧（${name}）"
  "${PY}" -m iroha_dataset build-readings --config "${CONFIG}" --set raw_dir="${WORK}/raw" --set data_dir="${dir}"
