...
| Shift+英字 | 英字入力（ことえり互換）。文章の途中でも確定せずに英単語を挿入できる（下記） |
...
以降は小文字・数字・記号も英字のまま末尾に足され、Enterでそのまま確定する
（`kyouha` `Shift+M` `ac` Enter → 「今日はMac」。英数モードへの切り替えや途中確定は不要）。
Spaceを押すと英字の文節が選ばれ、もう一度Spaceで 打鍵通り / 小文字 / 大文字 / 先頭だけ大文字 / 全角 の候補が出る。
Backspaceは英字を1文字ずつ消し、英字が無くなるとShiftを押す前の状態に戻る。Escは英字入力だけを取り消す。
