...
- **AI確定**: 修飾キー+Returnで、未確定文字列を英訳して確定／自分で書いたプロンプトでAI変換して確定（Apple Intelligence / Ollama / LM Studio）
...
| ⌃Return（設定で変更可） | 未確定文字列を英訳して確定（処理中はEscで取消） |
| 修飾キー+Return（既定オフ） | 未確定文字列をユーザ設定のプロンプトでAI変換して確定 |
...
| AI | 英訳して確定・AI変換して確定（プロンプト）・AIサービスの選択 |
...
修飾キー+Returnで、未確定文字列をAIに渡して結果を確定する。2種類あり、
それぞれ別のショートカットを割り当てる（同じキーは選べない）:

- **英訳して確定**（既定 ⌃Return）: 未確定文字列を英訳して入力する
- **AI変換して確定**（既定オフ）: 設定したプロンプトで処理して入力する。
  敬語化・要約・言い換えなど用途は自由。プロンプトに `{text}` と書くとその位置に
  未確定文字列が入る（無ければプロンプトに続けて渡される）

```
あした会議やるから来て
  ⌃Return（英訳）      -> Come to the meeting tomorrow.
  AI変換（丁寧な文体） -> 明日の会議に参加してください。
```

どちらも設定の「AIサービス」で選んだバックエンドを共有する:
