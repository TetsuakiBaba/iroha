...
| Tab | うすく表示された予測（予測変換・インライン補完、設定でON）を取り入れる |
...
- **予測変換（入力中）**: 入力を約300ms止めると、ライブ変換の結果に続く次の文節をカーソルの右に
  うすく表示する（例:「今日は」→ うすく「いい」）。Tabで取り入れると未確定文字列の一部になり、
  そのまま入力を続けられる（続けてTabを押すと文が伸びる）。取り入れた部分は読みを持たないので、
  Backspaceで予測ごと取り消し（Tabを押す前の表示に戻る）、Escでかなに戻す対象からは外れる。
  取り入れた部分を含む確定は学習しない。ライブ変換がONのときだけ動く
- **インライン補完（確定後）**: 確定して約300ms操作を止めると、確定した文章の続き（次の文節）を
  うすく表示する。Tabで確定、それ以外のキーではまず消えてからそのキーが普通に処理される
  （Escは消すだけ）。クリックやフォーカス移動では取り込まれない
...
- 予測変換・インライン補完は [PredictionEngine](macos/Sources/IrohaCore/PredictionEngine.swift)
  ...
  「確定済み文字列」（インライン補完）。コントローラは休止時間（既定300ms、`predictionDelayMs`）を
  キー入力の時刻から測り、ライブ変換の到着後に残り時間だけ待ってから予測を走らせる。
  うすい表示は未確定文字列（`setMarkedText`）の末尾に下線なし・薄い色で付ける
  （インライン補完では未確定文字列全体がうすい表示で、カーソルはその手前）
