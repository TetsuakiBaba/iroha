...
| タブ | 内容 |
|---|---|
| 入力 | ライブ変換・句読点で自動確定・候補数・予測変換・インライン補完・予測の休止時間・句読点スタイル（、。/ ，．） |
| 辞書 | ユーザ辞書（編集・macOSからの取り込み）・変換ルール（編集）・学習（ON/OFF・リセット）・学習用データの記録（ON/OFF・削除） |
| AI | AI変換して確定のプリセット3つ（名前・プロンプト・ショートカット）・選択テキストのAI編集・選択した文字数の表示・AIサービスの選択 |
...
### 学習用データの記録（設定でON、既定OFF）

上の学習とは別に、確定した変換を将来の変換モデルの追加学習に使うデータとして1件ずつ記録できる
（**設定 > 辞書 > 学習用データの記録**）。修正の有無にかかわらず、ライブ変換の確定・文節変換の確定・
...
- 学習用データの記録は `<データフォルダ>/logs/conversions/`
