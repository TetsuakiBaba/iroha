...
| ⌃Return（設定で変更可） | 未確定文字列を英訳して確定（Apple Intelligence / Ollama / LM Studio、翻訳中はEscで取消） |
...
英訳して確定の翻訳サービスは Apple Intelligence（オンデバイス、macOS 26+）/
Ollama / LM Studio から選択でき、Ollama / LM Studio では利用可能なモデルを
自動取得して一覧から選ぶ（thinking対応モデルでも思考過程は出力されない）。
