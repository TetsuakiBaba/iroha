...
- 学習はこの Mac の GPU（MLX）で行い、どこにも送信しない。記録 400 件で 30 秒ほど
- アダプタを使っている間に記録された確定も分け隔てなく学習に使う
  （記録は「ユーザがこれでよいと思って確定した結果」なので、どのモデルが候補を出したかは関係ない）
- 最初に記録を1件ずつ変換し直して、**いまのモデルが間違えるもの**を選り分ける（記録の `proposed` は
  辞書ラティスや学習を含むエンジン全体の提示なので、モデル自身の誤りとは一致しない。
  実測では「自分が直した 11 件」と「モデルが間違える 12 件」は半分しか重ならなかった）。
  学習に使うのは間違えた記録で、数が少ないので 1 件を 8 回ずつ繰り返して水増しし、
  正解できた記録を（水増し後の行数の 2 倍だけ）混ぜる。学習は「自分の左文脈とよみ」を条件に
  「自分が確定した文字列」を当てる形なので、正解できていた記録も自分の文脈・言葉づかいを
  学ばせる材料になる（間違いだけで学習すると偏った例に寄り、できていた変換が崩れる）
- 結果は**2つの数字**で出る。「間違えていた変換が当たるようになった数」（効果）と
  「元から正しかった変換を維持できた数」（副作用）。体感では悪化の方が目立つので必ず両方見る。
  間違えた記録の新しい方 2 割は学習に使わず、この評価に回す
...
- 対応するのは zenz（gpt2 アーキテクチャ）のモデル。記録 30 件以上・そのうちモデルが間違えるもの
  5 件以上で学習できる
