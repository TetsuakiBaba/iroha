...
from iroha.jsonlio import count_lines, read_json, write_json
from iroha.paths import Paths, SPLITS
from iroha.sources.base import all_source_info
...
def render_report(stats: dict) -> str:
    ...
    A(f"生成: {stats['generated_at']}　/　iroha-dataset {stats['iroha_version']}　"
      f"/　seed {stats['config']['seed']}\n")
    A("このファイルは `python -m iroha stats` が生成する。数値の定義は "
      "`data/stats.json` が正。\n")
