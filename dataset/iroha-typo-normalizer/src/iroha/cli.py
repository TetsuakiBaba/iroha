"""コマンドライン。
    ...
    python -m iroha download
    python -m iroha preprocess
    python -m iroha build-kkc      # KKC = かな漢字変換（Kana-Kanji Conversion）
    python -m iroha build-typo
    python -m iroha build-jwtd     # JWTD（Wikipedia の実 typo）→ 学習データ + ベンチ
    python -m iroha stats
    python -m iroha samples
    python -m iroha build-all      # download 以降を一括
    python -m iroha sources        # 使えるソースとライセンスを出す
...
from iroha.paths import paths_from_config
from iroha.sources import adapter_names, all_source_info, enabled_adapters, get_adapter
...
def cmd_build_kkc(args, cfg, paths) -> int:
    from iroha.kkc.build import KkcBuilder
...
def cmd_build_typo(args, cfg, paths) -> int:
    from iroha.typo.build import TypoBuilder
...
def cmd_build_jwtd(args, cfg, paths) -> int:
    from iroha.wild.jwtd import JwtdBuilder
    paths.ensure()
...
def cmd_estimate_typo_dist(args, cfg, paths) -> int:
    ...
    from iroha.wild.jwtd_dist import estimate
...
def cmd_stats(args, cfg, paths) -> int:
    from iroha.stats import run_stats
    paths.ensure()
