...
* **ja_aozorabunko** … 青空文庫（globis-university/aozorabunko-clean 由来）。
  ...
  （旧仮名は読みが現代かなと合わない。iroha/sources/aozora.py と同じ方針）
...
from iroha.download.http import download_stream_to, http_get
from iroha.sources.aozora import strip_aozora_markup
    ...
    def _paths(self) -> list[Path]:
        ...
        if not paths:
            raise FileNotFoundError(
                ...
                f"`python -m iroha download --source {self.name}` を実行する")
        return paths
