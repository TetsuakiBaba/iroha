...
///
/// - 予測変換: 入力中にキー入力が休止したら、未確定文字列の続き（次の文節）をうすく表示し、Tabで取り入れる
/// - インライン補完: 確定後にキー入力が休止したら、確定した文章の続きをうすく表示し、Tabで確定する
///
...
enum PredictionSettings {
    ...
    /// キー入力の休止とみなす時間（ミリ秒）。UIには出さない（UserDefaultsで調整可）
    ...
    static var idleDelay: Duration {
        ...
        let milliseconds = stored == 0 ? 300 : min(2000, max(100, stored))
