...
/// 記録を「いまのモデルが間違えるもの」と「正解できるもの」に振り分ける（ベースモデル・アダプタなし）。
///
/// 用途は**評価用の取り分け**と、記録のうちモデルがどれだけ間違えるかの表示。訓練には全件を使う。
/// 追加学習で変えられるのはニューラルネット（`ZenzEngine`）の出力だけなので、「間違い」は
/// **NN が実際に間違えた記録**で数えなければならない。記録の `proposed` / `edited` は辞書ラティスや
