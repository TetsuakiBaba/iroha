...
| iroha-llmjp-150m-full-Q5_K_M | 92.5% | 1.99% | 68.5% (137/200) | 4.63% | 35.6ms | 70.5% (141/200) | 4.26% | 96.7ms |

- 文脈なし/あり（各100件）の内訳: small NNのみ 89/80、辞書+NN 87/80。xsmall NNのみ 70/65、辞書+NN 71/67。
  llmjp-full NNのみ 73/64。f16 は NNのみ 72.0%（78/66）
- 自作モデルの値は下記「ReadingConstraint の語頭スペース拒否」修正後（同日）。修正前は NNのみ 59.0%、
  辞書+NN 72.0%、eval.tsv 67.5% だった
- レイテンシは同時に動いている処理で±30ms程度ぶれる。モデル間の相対比較にだけ使う
- eval.tsv は自分の文なので学習データが乗ると上振れする（llmjp-full は学習込みの旧計測 72.5% →
  空にすると 67.5%。制約修正後は 92.5%）。2026-09-16 より前の eval.tsv の値は比較に使わない
- zenz は辞書を足してもほぼ変わらない（small は2件落ち: 名付けた/名づけた、後10年→50年。xsmall は3件直り0件悪化）。
  自作モデルは制約修正前は辞書で+13pt（group/解説→開設 など27件）だったが、修正後はその大半を
  生成自身で正解するようになり、辞書の寄与は +2pt に縮んだ
...
#### ReadingConstraint が語頭スペースを弾いていた（2026-09-16 判明・同日修正）

学習マシン側（llama-cpp-python、辞書・制約なし、文脈40字）で同じ200件を測ると自作モデルだけ
Mac側「NNのみ」より11.5pt高く、zenz は一致した（small 84.0 vs 84.5、xsmall 67.5 vs 67.5）。
原因は `ReadingConstraint`（`IROHA_LATTICE=off` でも常に有効なトークンマスク）の空白の扱い。
llm-jp 語彙 99,584 語のうち 43,267 語は語頭マーカー「▁」付きで、出力時に先頭スペースになる。
制約は空白を「読みにそのまま現れるべき文字」として位置照合していたため、語頭トークンが全部弾かれ
（「▁グループ」が通らず「group」へ逸れる、「▁開設」が通らず「解説」になる等）、
2万語余りは「▁」付きしか語彙にないので制約下では出せなかった。文字単位語彙の zenz には▁が無く無影響。

修正: 空白（半角・全角スペース、タブ改行、U+2581）は読みの同じ位置にあれば消費し、なければ
消費なしで通す（`ReadingConstraint.isWhitespace`）。制約を丸ごと切る実験フラグ
`IROHA_NO_CONSTRAINT=1`（`ZenzEngine(usesReadingConstraint:)`）も追加した。

| モデル（NNのみ） | 修正前 | 制約なし | 修正後（制約あり） | Python 制約なし |
|---|---|---|---|---|
| iroha-llmjp-150m-full Q5_K_M | 59.0% | 68.5% | **68.5%** | 70.5% |
| iroha-llmjp-150m-full f16 | 61.0% | 72.0% | **72.0%** | 73.5% |
| zenz-v3.1-small Q5_K_M | 84.5% | 84.5% | 84.5% | 84.0% |
| zenz-v3.1-xsmall Q5_K_M | 67.5% | — | 67.5% | 67.5% |

修正後は制約ありでも制約なしと同じ値になり、制約の損失はゼロ。Python との残り約2ptは
`ChunkedConversionEngine`（50文字超の33件を分割変換。Pythonは一括生成）か Metal/CPU の数値差が
候補で未検証。新しい語彙のモデルを評価するときは `IROHA_NO_CONSTRAINT=1` と既定を比べて
制約の損失がないことを先に確認する。
